...
public class RoutingExceptionTest
{
    ...
    public void setUp()
    {
        ...
        {
            Logger.getLogger(RoutingExceptionTest.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        catch(AbstractException ex)
        {
            try
            {
                String result = $(m.getSOAPMessage().getSOAPBody().getOwnerDocument())
                        ...
                        .xpath("//ota:OTA_ErrorRS[@ErrorCode = '5555']")
                        .attr("ErrorMessage");
                assertEquals("Rounting failed - check destinations", result);
            }
            catch (SOAPException ex1)
            {
                Logger.getLogger(RoutingExceptionTest.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
    }
